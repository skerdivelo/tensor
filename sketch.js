function setup() {
    ...
    //----------------------------------------------------
    //this is done with promisies
    /* tense.data().then(function(stuff){
        console.log(stuff);
    }) */
    tense.print(); //stampa come una matrica/vettore
    console.log(tense.dataSync()); //same thing as the promise. Stampa come un array
    //----------------------------------------------------
    //se io volessi cambiare qualcosa dal tensor non posso farlo, posso creare una variabile dove copiare il tensor
    const varTensor = tf.variable(tense);
    console.log(varTensor.dataSync());
}
